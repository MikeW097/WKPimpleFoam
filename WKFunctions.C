double dt;
int N_OUTLETS;
DynamicList<string> patch_names(20); // 20 has been set as the maximum limit of outlets that are expected
...
void Wk_pressure_update_unsteady(int i, fvMesh & mesh, surfaceScalarField & phi, scalarIOList & store)
{
...
void Wk_pressure_update_steady(int i, fvMesh & mesh, surfaceScalarField & phi, scalarIOList & store)
{
	wk[i].Q_current = calculate_flow_rate(i,mesh,phi);
	scalar P_current = wk[i].Q_current*(wk[i].Z+wk[i].R)+wk[i].Pout_current;
	wk[i].P_current = P_current;
...
void execute_at_end(fvMesh & mesh, surfaceScalarField & phi, scalarIOList & store, const word & simType="unsteady")
{
	for (int i=0;i<N_OUTLETS;i++)
	{
		if (simType == "steady")
		{
			...
			Wk_pressure_update_steady(i, mesh, phi, store);
		}
		else if (simType == "unsteady")
		{
			...
			Wk_pressure_update_unsteady(i, mesh, phi, store);
		}
	}
